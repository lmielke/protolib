...
# use --port to override: proto server --port 9005
...
def _get_tts_engine(*args, **kwargs):
    global _tts_engine
...
class ProtoControlHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def load_apis(cls, *args, **kwargs):
        ...
        for filename in os.listdir(apis_dir):
            ...
            try:
                module_path = f"protolib.apis.{api_name}"
    ...
    def do_POST(self, *args, **kwargs):
        """Handle POST requests. Routes /announce to the announce API."""
        parsed_url = urlparse(self.path)
        api_name = parsed_url.path.strip("/")
        if api_name == "announce":
            try:
                ...
                from .announce import _handle_registration
                if not getattr(sts, 'registry_host_enabled', False):
                    self._send_json_response({"error": "announce API is not enabled"}, 403,
                                             *args, **kwargs)
                    return
                state = _handle_registration(body, *args, **kwargs)
                self._send_json_response(state, 200, *args, **kwargs)
            except Exception as e:
                self.send_error(500, f"Error handling POST to /announce: {e}")
                logging.error(f"POST /announce error: {e}")
        else:
    ...
    def _send_ok_response(self, content: str, *args, **kwargs):
        """Sends a 200 OK response with ANSI codes converted to HTML."""
...
def run_server(*args, port: int = None, verbose: int = 1, **kwargs):
    ...
    # Start registry heartbeat in background (silent fail if registry unreachable)
    try:
        ...
        logging.info("Registry heartbeat started")
    except Exception:
        logging.warning("Registry heartbeat not started")

    # Start TTL sweep if this instance is the registry host
    if getattr(sts, 'registry_host_enabled', False):
        try:
            from ..registry import RegistryHost
            host = RegistryHost(*args, **kwargs)
            host.start_sweep_loop(*args, **kwargs)
            logging.info("Registry host TTL sweep started")
        except Exception:
            logging.warning("Registry host sweep not started")

    handler = ProtoControlHandler
    with socketserver.TCPServer(("", port), handler) as httpd:
        startup_message = f"{sts.package_name} control server starting on port {port}"
        logging.info(startup_message)
        logging.info(f"Available API endpoints: {list(handler.available_apis.keys())}")
        logging.info(f"{Fore.YELLOW}Uri:{Fore.RESET} http://localhost:{port}/info/?infos=package")

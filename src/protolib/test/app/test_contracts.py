...
description: >-
  Runs integration tests for the app/contracts module, verifying kwarg cleaning, validation,
  and path normalization logic. Covers functions like clean_kwargs, check_missing_kwargs,
  and normalize_path to ensure correct behavior. Consumed by the test suite to validate contract
...
"""
import os, unittest
from protolib.app.contracts import Contracts


class TestCleanKwargs(unittest.TestCase):
    """Contracts.clean_kwargs: strip whitespace from keys and string values."""

    def test_strips_string_values(self, *args, **kwargs):
        result = Contracts().clean_kwargs(name="  hello  ", count=5)
        assert result["name"] == "hello"

    def test_strips_single_quotes(self, *args, **kwargs):
        result = Contracts().clean_kwargs(name="'world'")
        assert result["name"] == "world"

    def test_preserves_non_strings(self, *args, **kwargs):
        result = Contracts().clean_kwargs(count=42, flag=True)
        assert result["count"] == 42
        assert result["flag"] is True

    def test_strips_key_whitespace(self, *args, **kwargs):
        result = Contracts().clean_kwargs(**{" key ": "val"})
        assert "key" in result


class TestCheckMissingKwargs(unittest.TestCase):
    """Contracts.check_missing_kwargs: validate required args per API."""

    def test_non_clone_api_passes(self, *args, **kwargs):
        Contracts().check_missing_kwargs(api="info")
...
class TestGetPackageData(unittest.TestCase):
    """Contracts.get_package_data: derive paths from settings."""

    def test_returns_dict_with_keys(self, *args, **kwargs):
        result = Contracts().get_package_data()
    ...
    def test_custom_work_dir(self, *args, **kwargs):
        result = Contracts().get_package_data(work_dir="/tmp")
        assert result["work_dir"] == "/tmp"


class TestNormalizePath(unittest.TestCase):
    """Contracts.normalize_path: canonicalize user-supplied paths."""

    def test_expands_home(self, *args, **kwargs):
        result = Contracts().normalize_path("~/test")
        assert "~" not in result
        assert os.path.isabs(result)

    def test_resolves_relative(self, *args, **kwargs):
        result = Contracts().normalize_path("relative/path")
        assert os.path.isabs(result)

    def test_empty_returns_empty(self, *args, **kwargs):
        result = Contracts().normalize_path("")
        assert result == ""

    def test_absolute_unchanged(self, *args, **kwargs):
        result = Contracts().normalize_path("/usr/local/bin")
        assert result == "/usr/local/bin"


class TestCleanPaths(unittest.TestCase):
    """Contracts.clean_paths: normalize all _dir/_path kwargs."""

    def test_normalizes_dir_keys(self, *args, **kwargs):
        result = Contracts().clean_paths(work_dir="~/test", name="keep")
        assert "work_dir" in result
        assert "name" not in result

    def test_normalizes_path_keys(self, *args, **kwargs):
        result = Contracts().clean_paths(file_path="relative/file.py")
        assert os.path.isabs(result["file_path"])


class TestChecks(unittest.TestCase):
    """Contracts.checks: full pipeline clean+validate+enrich."""

    def test_returns_enriched_kwargs(self, *args, **kwargs):
        result = Contracts().checks(api="info", verbose=0)
        assert "work_dir" in result
        assert "pg_name" in result

"""
    ...
    parses protolib arguments and keyword arguments
    args are provided by a function call to mk_args()

    RUN like:
    import protolib.arguments
    kwargs.update(arguments.mk_args().__dict__)

    NOTE: Avoid douplicate use of existing argument for unrelated themes. If in doupbt, add a new argument.
...
def mk_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Registry URL to register with (default: from settings.yml registry_port)",
    )
    ...
    parser.add_argument(
        ...
        type=bool,
        default=None,
        help="skip confirmation prompts during clone (e.g. project name confirmation)",
    )
